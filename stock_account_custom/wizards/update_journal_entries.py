        ...
        for move in self.account_move_ids:
            ...
            if move.stock_move_id:
                ...
                if move.stock_move_id.purchase_line_id and move.stock_move_id.purchase_line_id.discount and move.stock_move_id.purchase_line_id.discount != 0 and move.stock_move_id.price_unit:
                    ...
                    new_price = move.stock_move_id.product_id.standard_price
                    for supplier in move.stock_move_id.product_id.product_tmpl_id.seller_ids:
                        if supplier.name.id != 1:
                            price = supplier.price * (
                                    1 - (move.stock_move_id.product_id.product_tmpl_id.supplier_discount / 100))
                            new_price = supplier.currency_id.with_context(date=move.date).compute(price,
                    ...
                    for move_line in move.line_ids:
                        if move_line.credit != 0:
                            self._cr.execute("""UPDATE  account_move_line set credit=%s, balance=%s WHERE id=%s""", (
                            new_price * move.stock_move_id.quantity_done, new_price * move.stock_move_id.quantity_done,
                            move_line.id))
                        if move_line.debit != 0:
                            self._cr.execute("""UPDATE   account_move_line set debit=%s, balance=%s WHERE id=%s""", (
                            new_price * move.stock_move_id.quantity_done, new_price * move.stock_move_id.quantity_done,
                            move_line.id))
                    ...
                    if move.stock_move_id.product_id.supplier_discount and move.stock_move_id.picking_type_id.code == 'outgoing':
                        ...
                        for supplier in move.stock_move_id.product_id.product_tmpl_id.seller_ids:
                            if supplier.name.id != 1:
                                ...
                                new_price = supplier.currency_id.with_context(date=move.date).compute(price,
                                                                                                      move.company_id.currency_id)
                        move.stock_move_id.write({'price_unit': new_price * -1})
                        # if new_price < 0:
                        #     new_price = new_price * -1
                        move.button_cancel()
                        for move_line in move.line_ids:
                            if move_line.credit != 0:
                                self._cr.execute("""UPDATE  account_move_line set credit=%s, balance=%s WHERE id=%s""",
                                                 (new_price * move.stock_move_id.quantity_done,
                                                  new_price * move.stock_move_id.quantity_done, move_line.id))
                            if move_line.debit != 0:
                                self._cr.execute("""UPDATE   account_move_line set debit=%s, balance=%s WHERE id=%s""",
                                                 (new_price * move.stock_move_id.quantity_done,
                                                  new_price * move.stock_move_id.quantity_done, move_line.id))
                    ...
                    for move_line in move.line_ids:
                        if move_line.product_id:
                            ...
                            for supplier in move_line.product_id.product_tmpl_id.seller_ids:
                                if supplier.name.id != 1:
                                    price = supplier.price * (
                                                1 - (move_line.product_id.product_tmpl_id.supplier_discount / 100))
                                    price = supplier.currency_id.with_context(date=move.date).compute(price,
                                                                                                      move.company_id.currency_id)
                            if (
                                    move_line.account_id.id == move_line.product_id.product_tmpl_id.categ_id.property_account_expense_categ_id.id or move_line.account_id.code in expense_account_code) and move_line.debit != 0.0:
                                self._cr.execute("""UPDATE  account_move_line set debit=%s,balance=%s WHERE id=%s""",
                                                 (price * move_line.quantity, price * move_line.quantity, move_line.id))
...
class WizardDeleteAccountAccount(models.TransientModel):
    ...
    _description = 'Wizard Delete Account'
    ...
    def button_delete_account(self):
        for account in self.account_ids:
            ...
            for partner_prop_acc in partner_prop_accs:
                _logger.info(values)
                # 136->804, 154 -> 814
                account_replace = self.env['account.account'].search(
                    [('name', '=', account.name), ('deprecated', '=', False),
                     ('company_id', '=', account.company_id.id)])
                if account_replace:
                    partner_prop_acc.write({'value_reference': 'account.account,%s' % account_replace.id})
                    _logger.info('update value reference')
                # TODO: Check With Client If Needed, More Clarity Required
                # elif account.id == 136:
...
class WizardFixWowCreditCardReconciliation(models.TransientModel):
    ...
    _description = 'Wizard Fix CreditCard Reconciliation'
    ...
    def button_update_reconciliation(self):
        for wizard in self:
            ...
            for move in account_move_lines:
                ...
                move.__compute_amount_residual()
